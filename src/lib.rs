...
impl Into<ratatui::style::Color> for Color {
    fn into(self) -> ratatui::style::Color {
        match self {
            Color::Blank => ratatui::style::Color::DarkGray,
...
impl Default for WordleGrid {
    fn default() -> WordleGrid {
        WordleGrid {
            grid: [[WordleBox::new(None, Color::Blank); 5]; 6],
            first_free: (0, 0),
        }
    }
}
...
pub fn check_word(input: &str, word: &str) -> [WordleBox; 5] {
    let wboxes: Vec<WordleBox> = input
        .trim()
        .chars()
        .zip(word.chars())
        .map(|(i, w)| {
    ...
    let boxed_array: Box<[WordleBox; 5]> = match boxed_slice.try_into() {
        Ok(ba) => ba,
        Err(o) => panic!(
            "Expected a Vef of WordleBoxes of length {} but it was {}",
            5,
            o.len()
        ),
    };
